...
const Poll: React.FC = () => {
    ...
    const handleSendQuestion = () => {
        if (activeQuestionId !== null) {
            // Mock sending question. Here is where you'd use socket.io
    ...
    const handleShowAnswers = () => {
        const mockAnswers: Answer[] = [
            // Mock data based on the active question
            { studentName: 'Alice', questionId: activeQuestionId || 0, response: 'Answer A' },
        ];
    ...
    return (
        <div id='poll'>
            <h1>Poll</h1>
            <textarea onChange={handleMultipleQuestionsInput} placeholder="Paste questions here, one per line"></textarea><br/>
            ...
            <button className="btn btn-primary" onClick={handleSendQuestion}>Send Question</button><br/>

            <button className="btn btn-primary" onClick={handleShowAnswers}>Show Answers</button><br/>

            {answers.length > 0 && (
                <>
                    <StudentAnswers answers={answers}/>
                    {hasNextQuestion() && <button className="btn btn-primary" onClick={handleSendNext}>Send Next</button>}
                </>
            )}
        </div>
    );
